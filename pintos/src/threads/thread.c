...
/* Refresh the priority of the ready threads */
void
refresh_cpu (struct thread *t, void *aux UNUSED)
{
  if (t != idle_thread)
  {
    /*
...
{
  int ready_length = list_size (&ready_list);
  load_avg = add_fp (div_int (mult_int (load_avg, 59), 60), mult_int (div_int (int_to_fp(1), 60), ready_length) );
  int avg = fp_to_int_round(load_avg);
  printf("Soy load avg = %d\n", avg );
}


/* Compare 2 threads for wakeup time or priority */
bool
wakeup_cmp (const struct list_elem *left, const struct list_elem *right, void *aux UNUSED)
{
...
bool
priority_cmp (const struct list_elem *left, const struct list_elem *right, void *aux UNUSED)
{
